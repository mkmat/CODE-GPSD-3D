...
implicit none
    ...
    namelist /list/ ro,rp,rc,shots,N,quiet,more,np,TPSD
...
!    end
    ...
    allocate(coord(N,3))
    do id=1,N
        read(*,*) coord(id,:),voro_vertices,voro_faces,(itmp(face),face=1,voro_faces)
        read(*,*) 
    ...
    do id=1,N
        ...
        read(*,*) coord(id,:),voro_vertices,voro_faces,(voro_vertices_for_face(face),face=1,voro_faces)
    ...
    real*8  :: R
    real*8  :: r_is_above
    real*8  :: vp(3),P(3),Pn,center(3),PM(3)
...
!$omp parallel num_threads ( np) shared ( i1min, i1max, i2min, i2max ) private ( shot, myrand3, vp, P, Pn, o, icell, jcell, k, i, A, facenormal, B, C, tmp_r, tmp_center, r, center, X, Xn, PM, r_is_above )
...
!$omp do reduction (min : min_pore_radius) reduction (max : max_pore_radius) reduction( + : mean_pore_radius, count_r, std_pore_radius)
        do shot=1,shots
            ...
            do i1=i1min,i1max
             jcell(1) = icell(1)+i1
             if (jcell(1).eq.neighborlist_M(1)+1) jcell(1)=1
             if (jcell(1).eq.0) jcell(1)=neighborlist_M(1)
            do i2=i2min,i2max
             jcell(2) = icell(2)+i2
             if (jcell(2).eq.neighborlist_M(2)+1) jcell(2)=1
             if (jcell(2).eq.0) jcell(2)=neighborlist_M(2)
            do i3=i3min,i3max
             jcell(3) = icell(3)+i3
             if (jcell(3).eq.neighborlist_M(3)+1) jcell(3)=1
             if (jcell(3).eq.0) jcell(3)=neighborlist_M(3)
                ...
                do while (i.ne.0) 
                    if (R.lt.triangle_Rho(i)) then ! otherwise, we cannot improve R using this triangle
                        ...
                        r_is_above  = dot_product(P,facenormal)+rp  ! Pn+rp 
...
!$omp end parallel
    ...
    integer, dimension(:), allocatable      :: nextbead
        ...
        deallocate(neighborlist_firstT)
        deallocate(neighborlist_nextT)
